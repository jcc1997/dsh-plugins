# 动态插件专用知识（legacy，2026-08 起隔离）

> **背景**：插件开发已切换为正式 bundle + HMR 形态（见同目录 SKILL.md）。本文件是动态插件（`cordis_define` / `cordis_run`）的完整专用知识——受限环境约束、模板、SDK 零粘贴、动态踩坑、运行模型。**目前仅在会话内做快速原型/临时调试时参考**；正式功能一律走正式 bundle。cordis 工具（define/run/inspect/stop/undefine）仍然可用。

## 零、硬性规则（动态插件专用，违反 = 烧 token）
...
## 四、动态 TS 编译管线（esbuild iife → 受限函数体）
...
| **事件（仅监听）** | `ctx.on / ctx.once` 监听宿主事件：`slots/changed`、`credentials/updated`、tool 注册变更、`session/*`、`subagent/*`… | 动态插件**不能发事件**（见下） |
...
- **工具调用**：`ctx.tools` 是只读门面（schemas/get 只返回元数据），**禁止**插件代码直接调用其他插件的工具 execute（防绕过 ToolRuntime 守卫链）→ 插件间协作走服务，不走工具。
