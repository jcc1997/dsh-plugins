...
- **M3+ 关联卡片重构**：所有关联统一「关联」卡片按类型展示 + 删除；「+ 新增」折叠表单（GitHub 仓库/分支/MR/本地仓库/会话）；jira 已移除。**会话关联**（ref kind session）：点击 → 关闭看板 + `sessions.open(id)` 定位会话（client 服务实测可用）。**会话「任务」tab**：注册 `conversation.view`（session scope，组件收 sessionId）→ 展示当前会话关联 task 列表 → 点击打开可编辑详情（复用 CardDrawer + useKanbanBoard hook）。**槽位渲染授权 vs 服务通道**：`kanban.card.actions` 子槽位由 sidebar 条目声明（声明者独占 renderSlot），会话 tab 无法渲染该槽位 → 同步按钮走 `kanban/git-sync` 桥接 RPC（kanban host 内 `ctx.get('git').sync`，跨插件服务通道），已实现。
