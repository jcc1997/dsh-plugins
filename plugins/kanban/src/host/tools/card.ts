// host/tools/card.ts — Ticket类 10 个 agent 工具：查询（view/get_card/search/recent）+ 操作（create/move/update/tags/comment/delete）
    ...
    },
    {
      ...
      execute: async (args: any, exec: any) => {
        return mutateBoard(fs, (board: any) => {
          ...
          appendActivity(card, '创建Ticket' + (tpl ? '（模板：' + tpl.name + '）' : '') + (sessionId ? '，关联本会话' : ''))
          col.cards.push(card)
    ...
    },
    {
      ...
      execute: async (args: any) => {
        return mutateBoard(fs, (board: any) => {
          ...
          if (changed) { card.updatedAt = now(); appendActivity(card, '更新Ticket') }
