...
门禁是**门禁库（board.gateLibrary）里的独立实体**：单独创建、单独配置，Ticket与模板按 id **勾选引用**（card.gateIds / template.gateIds）。同一门禁可被多张卡/多个模板复用，改一处全局生效。
...
| 函数 | 能力 | 示例 |
|---|---|---|
| `gate.card({})` | 当前被检查的Ticket（完整 JSON） | `const c = await gate.card({})` |
| `gate.getCard({cardId})` | 读任意Ticket | `await gate.getCard({cardId: 'k…'})` |
