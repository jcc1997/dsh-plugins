...
async function run() {
  ...
  const opts = watch
    ? { ...base, watch: { onRebuild: async (err) => {
        ...
        try { await toPluginBody(join(root, 'dist', 'client.js'), '__KB__'); console.log('[git] client rebuilt -> dist/client.js') } catch (e) { console.error('post failed', e) }
      } } }
    : base
  ...
  console.log('[git] client.js written')
  ...
  await build({
    ...
    ...(watch ? { watch: { onRebuild: async (err) => {
      ...
      try { await toPluginBody(join(root, 'dist', 'host.js'), '__KBH__'); console.log('[git] host rebuilt -> dist/host.js') } catch (e) { console.error('post failed', e) }
    } } } : {}),
  })
  if (!watch) await toPluginBody(join(root, 'dist', 'host.js'), '__KBH__')
  console.log('[git] host.js written')
  if (watch) console.log('[git] watching src/ ...')
}
...
async function makeSubmit() {
  ...
  console.log('[git] submit.json +', idx, 'segments ready (total', j.length, 'bytes)')
}
