...
8. **批注/引用类操作**: 划词批注为**内嵌在对应块下方的批注框**(文档流;引用风格=左品牌蓝竖条+淡灰底+左右 padding);引用文字带**行号范围**纯文字(块级 data-mdr-line/data-mdr-line-end 1-based,格式 `"划中文字" #L11-13`,单行 `#L11`,随批注提交便于 agent 回原文定位);右侧标注支持**点击定位原文**(scrollIntoView+高亮闪烁)/**编辑**(预填重开批注框,保存更新原条)/**删除**;compact 输入,Enter 添加,取消关闭(ADR-10/11)。
