...
3. **禁 input 框式设计（红线）** — 富文本编辑器、标题、描述等「内容类」输入一律 Notion 式：无边框、无背景、无 focus ring 变色；靠 placeholder 提示与行 hover 高亮表达可编辑性。仅评论、标签、路径等「短输入」控件允许保留轻边框（border-l2 + focus ring）。工具栏分隔用虚线而非实线。
4. **反馈即时** — hover 轻量（背景/边框变化），active 明确，拖拽有可见轨迹，保存有状态提示。
5. **一致性优先** — 所有组件共用同一套 token；同义操作（主按钮/次按钮/危险按钮）全局唯一形态。
6. **克制动效** — 仅过渡（150ms ease）与浮层出现（240ms）；图标旋转等状态指示动画仅限 loading 场景，且用 CSS 动画实现（不引入 JS timer）。
